...
export default function UserInvestStepThree({
  ...
  const updateRowAmount = (title, value) => {
    const num = Number(String(value).replace(/[^0-9.-]+/g, '')) || 0;
    setRows((prev) =>
      prev.map((r) => (r.title === title ? { ...r, amount: Math.max(0, Math.round(num)) } : r))
    );
  };

  const removeRow = (title) => setRows((prev) => prev.filter((r) => r.title !== title));
    ...
    <div>
      ...
      {totalInvested > planAmount && (
        <BodyBase>
          ...
          , reaching <span className="font-bold text-2xl text-btext-1-dark">${totalInvested}</span>
        </BodyBase>
      )}
      <div className="mt-6 space-y-4">
        {rows.map((p) => {
          const pct =
            totalInvested > 0 ? Math.round((Number(p.amount || 0) / planAmount) * 100) : 0;
          return (
            <div
              key={p.title}
            ...
            >
              <div className="flex items-center gap-3 w-full sm:min-w-[35%]">
                <div className="w-14 h-14 bg-gray-100 rounded-md flex items-center justify-center overflow-hidden">
                  <img src={AssureImp} alt={p.title} className="w-full h-full object-cover" />
                </div>

                <div className="min-w-0">
                  <BodyBase fontWeight={`font-semibold`}>{p.title}</BodyBase>
                  <BodySmall textColor={`text-btext-1-base`}>{p.developer}</BodySmall>
                </div>
              </div>
              <div class="hidden sm:block w-0 h-8 outline outline-1 outline-offset-[-0.50px] outline-border-primary"></div>
              ...
              <div class="hidden sm:blockw-0 h-8 outline outline-1 outline-offset-[-0.50px] outline-border-primary"></div>
              <button
                onClick={() => removeRow(p.title)}
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <RoundedButton type="button" label={'Prev'} bg={'bg-white'} onClick={onPrev} />
        <RoundedButton
          type="button"
          label={'Invest'}
